...
import { useCustomerByUserId, useCreateMyOrder, useMyDeliveryOrders, useMyDeliveryVehicles, useMyOrders, useUpdateMyOrder } from '../../hooks/queries/useCustomers';
...
const GROCERY_SENDER_CREATE_PATH = '/app/don-hang-cua-toi/tao-don-gui';
...
const getOrderPrimaryProductName = (order: ImportOrder) => {
  const firstItem = order.import_order_items?.[0];
  return firstItem?.products?.name || firstItem?.package_type || order.notes || 'Đơn hàng';
};

const getOrderTotalQuantity = (order: ImportOrder) => {
  const itemTotal = order.import_order_items?.reduce((sum, item) => sum + Number(item.quantity || 0), 0) || 0;
  return itemTotal || Number(order.quantity || 0) || 1;
};

const mapCustomerOrderToDeliveryOrder = (order: ImportOrder): DeliveryOrder => ({
  id: `customer-order:${order.id}`,
  import_order_id: order.order_category === 'vegetable' ? undefined : order.id,
  vegetable_order_id: order.order_category === 'vegetable' ? order.id : undefined,
  product_name: getOrderPrimaryProductName(order),
  total_quantity: getOrderTotalQuantity(order),
  delivered_quantity: 0,
  remaining_quantity: getOrderTotalQuantity(order),
  unit_price: order.unit_price,
  order_category: order.order_category || 'standard',
  status: 'hang_o_sg',
  ...
  delivery_vehicles: [],
  payment_collections: [],
  import_orders: order.order_category === 'vegetable' ? undefined : {
    ...
    admin_confirmed_at: order.admin_confirmed_at || null,
    total_amount: order.total_amount,
    payment_status: order.payment_status,
    profiles: order.profiles,
    selected_alias: order.selected_alias || null,
    receipt_image_url: order.receipt_image_url || null,
    receipt_image_urls: order.receipt_image_urls,
    import_order_items: order.import_order_items,
    deleted_at: order.deleted_at || null,
  },
  vegetable_orders: order.order_category === 'vegetable' ? {
    order_code: order.order_code,
    sender_name: order.sender_name,
    sender_id: order.sender_id || null,
    receiver_name: order.receiver_name,
    ...
    total_amount: order.total_amount,
    payment_status: order.payment_status,
    profiles: order.profiles ? { full_name: order.profiles.full_name } : undefined,
    selected_alias: order.selected_alias || null,
    receipt_image_url: order.receipt_image_url || null,
    receipt_image_urls: order.receipt_image_urls,
    import_order_items: order.import_order_items,
...
const GrocerySenderOrdersPage = () => {
  const navigate = useNavigate();
  const customerType = 'grocery_sender' as CustomerOrdersPageType;
  const { user } = useAuth();
  ...
  const { data: myOrders, isLoading: loadingMyOrders } = useMyOrders(!!user?.id);
  ...
  const sortedOrders = useMemo(() => {
    if (!customerId || !orderPolicy) return [];
    const customerDeliveryOrders = (deliveryOrders || []).filter((order) => {
      const sourceOrder = getDeliverySourceOrder(order);
    ...
    const deliverySourceIds = new Set(
      customerDeliveryOrders
        .map((order) => order.import_order_id || order.vegetable_order_id)
        .filter((id): id is string => Boolean(id)),
    );

    const pendingCustomerOrders = (myOrders || [])
      .filter((order) => order.order_category === orderCategory)
      .filter((order) => !deliverySourceIds.has(order.id))
      .filter((order) => {
        if (isSenderCustomer) return order.sender_id === customerId;
        return order.customer_id === customerId;
      })
      .map(mapCustomerOrderToDeliveryOrder);

    return groupCustomerDeliveryOrders([...pendingCustomerOrders, ...customerDeliveryOrders]).sort(
      (a, b) => {
        return (
  ...
  }, [customerId, deliveryOrders, isSenderCustomer, myOrders, orderCategory, orderPolicy]);
  ...
  const openCreateModal = () => {
    navigate(GROCERY_SENDER_CREATE_PATH);
    return;
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload = {
      ...
      notes: formState.notes || 'Đơn hàng khách gửi tạp hóa',
  ...
  if (loadingCustomer || isLoading || loadingMyOrders) {
    return (
      <div className="w-full flex-1">
        <PageHeader title="Đơn hàng của tôi" description="Theo dõi đơn hàng tạp hóa bạn đã gửi" />
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 w-full flex-1 flex flex-col min-h-0 bg-slate-50/60 -m-4 p-4 md:-m-6 md:p-6">
      <PageHeader
        title="Đơn hàng của tôi"
        description="Tạo đơn hàng tạp hóa và theo dõi trạng thái giao hàng, cước phí"
        backPath="/"
      />
      ...
      <div className="space-y-4 flex-1 min-h-0 flex flex-col">
        <div className="bg-card rounded-xl border border-border shadow-sm p-3 md:p-4">
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-primary px-4 text-[13px] font-bold text-white shadow-sm transition-colors hover:bg-primary/90 disabled:opacity-60"
            title={canSelfCreate ? 'Tạo đơn hàng' : 'Bạn chưa có quyền tạo đơn hàng'}
          >
            <Plus size={16} />
          ...
          <div className="min-w-0 flex-1">
            <SearchInput
              placeholder="Tìm đơn hàng, người gửi, người nhận..."
              onSearch={(raw) => setSearchQuery(raw)}
              className="h-10 rounded-lg bg-muted/10"
            />
          </div>

          <div className="hidden items-center gap-2 shrink-0 rounded-lg border border-border/80 bg-muted/20 px-3 py-2 text-[12px] font-bold text-muted-foreground lg:flex">
            <ShieldCheck size={15} />
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm flex flex-col flex-1 min-h-0 overflow-hidden">
          <div className="flex flex-col shrink-0 border-b border-border bg-white">
            <div className="grid grid-cols-3 gap-2 px-3 py-3 md:flex md:items-center md:overflow-x-auto custom-scrollbar">
              {(['in_sg', 'processing', 'delivered'] as OrderStatusFilter[]).map((status) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-center md:justify-start gap-1.5 rounded-lg px-2 py-2 text-[11px] md:px-4 md:text-[12px] font-bold transition-all whitespace-nowrap ${
            ...
            <form onSubmit={handleSubmit} className="h-full flex flex-col">
              <div className="px-5 py-4 border-b border-border bg-muted/20">
                ...
                <p className="text-[12px] text-muted-foreground mt-1">
                  Đơn hàng tạp hóa · {isSenderCustomer ? 'Bạn là người gửi' : 'Bạn là người nhận'}
                </p>
              </div>
              ...
              <div className="p-5 space-y-5 overflow-y-auto flex-1 min-h-0">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[12px] text-muted-foreground">Nhập mặt hàng và số lượng cần gửi.</p>
                    </div>
                    <button
                      type="button"
                      onClick={addItem}
                ...
                <div className="space-y-1.5">
                  <label className="text-[12px] font-semibold text-muted-foreground">Ghi chú đơn hàng</label>
                  <textarea
                    rows={3}
              ...
              <div className="px-5 py-4 border-t border-border bg-muted/20 flex items-center justify-end gap-2">
                ...
                >
                  ...
                  {editingOrder ? 'Lưu cập nhật' : 'Tạo đơn hàng'}
                </button>
              </div>
            </form>
          </div>
...
const EmptyOrdersState: React.FC<{ canSelfCreate: boolean; onCreate: () => void }> = ({ canSelfCreate, onCreate }) => (
  <div className="mx-auto flex max-w-sm flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-muted/20 px-5 py-8 text-center">
    ...
    <p className="mt-1 text-[13px] text-muted-foreground">Tạo đơn hàng đầu tiên để gửi tạp hóa.</p>
    <button
      type="button"
      onClick={onCreate}
      disabled={!canSelfCreate}
