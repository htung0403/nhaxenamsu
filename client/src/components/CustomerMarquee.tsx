...
export function CustomerMarquee() {
  ...
  return (
    <section className="logistics-grid overflow-hidden border-y border-slate-200 bg-[#F3F6FA] py-20 md:py-28">
      ...
      <div className="flex overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_12%,black_88%,transparent)]">
        <motion.div className="flex min-w-max gap-4" animate={{ x: ['0%', '-33.333%'] }} transition={{ duration: 24, repeat: Infinity, ease: 'linear' }}>
          {marqueeItems.map(({ name, icon: Icon }, index) => (
            <div key={`${name}-${index}`} className="cargo-card flex min-w-64 items-center gap-4 rounded-full border border-white/90 bg-white/90 px-6 py-4">
