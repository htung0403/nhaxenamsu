...
export function Process() {
  return (
    <section id="process" className="bg-white px-4 py-24 md:py-32">
      <div className="mx-auto max-w-7xl">
        <motion.div className="mb-12 max-w-3xl" initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ duration: 0.7 }}>
          ...
          <h2 className="mt-5 text-4xl font-black tracking-[-0.04em] text-[#071A3D] md:text-6xl">Quy trình vận chuyển đơn giản</h2>
          <p className="mt-5 text-lg leading-8 text-slate-600 md:text-xl">Bốn bước rõ ràng giúp khách gửi hàng dễ theo dõi và yên tâm hơn.</p>
        </motion.div>
        ...
        <motion.div className="relative grid gap-5 md:grid-cols-4" initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-100px' }} variants={container}>
          ...
          {processSteps.map(({ title, description, icon: Icon }, index) => (
            <motion.article key={title} variants={card} className="relative flex items-start gap-4 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-950/[0.04] md:block md:p-6">
