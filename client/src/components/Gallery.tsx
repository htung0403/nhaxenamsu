...
export function Gallery() {
  ...
  return (
    <section id="gallery" ref={sectionRef} className="logistics-grid bg-[#F3F6FA] px-4 py-28 md:py-40">
      <div className="mx-auto max-w-7xl">
        <motion.div className="mb-12 max-w-3xl" initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ duration: 0.7 }}>
          ...
          <h2 className="font-headline mt-5 text-4xl font-black tracking-[-0.045em] text-[#071A3D] md:text-6xl">Hình ảnh hoạt động</h2>
        ...
        <motion.div style={{ y }} className="grid auto-rows-[230px] grid-flow-dense gap-5 md:grid-cols-3">
          {galleryItems.map((item) => (
            <motion.figure key={item.title} className={`cargo-card group relative overflow-hidden rounded-[2rem] bg-slate-200 shadow-xl shadow-blue-950/[0.08] ${item.className ?? ''}`} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.65 }} whileHover={{ y: -6 }}>
